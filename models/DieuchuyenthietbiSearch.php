<?php

namespace app\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\Dieuchuyenthietbi;

/**
 * DieuchuyenthietbiSearch represents the model behind the search form of `app\models\Dieuchuyenthietbi`.
 */
class DieuchuyenthietbiSearch extends Dieuchuyenthietbi
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['ID', 'ID_THIETBI', 'ID_TRAM_NGUON', 'ID_TRAM_DICH'], 'integer'],
            [['NGAY_CHUYEN', 'LY_DO'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Dieuchuyenthietbi::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'ID' => $this->ID,
            'ID_THIETBI' => $this->ID_THIETBI,
            'NGAY_CHUYEN' => $this->NGAY_CHUYEN,
            'ID_TRAM_NGUON' => $this->ID_TRAM_NGUON,
            'ID_TRAM_DICH' => $this->ID_TRAM_DICH,
        ]);

        $query->andFilterWhere(['like', 'LY_DO', $this->LY_DO]);

        return $dataProvider;
    }
}
